...
// Get random paper (for quick start)
router.get('/paper', authMiddleware, (req, res) => {
  const rows = db.prepare('SELECT * FROM questions ORDER BY RANDOM() LIMIT 30').all();
...
router.post('/submit', authMiddleware, (req, res) => {
  ...
  const transaction = db.transaction(() => {
    for (const a of answers) {
      const q = db.prepare('SELECT answer, type FROM questions WHERE id = ?').get(a.questionId);
      let isCorrect;
      if (q && q.type === 'coding') {
